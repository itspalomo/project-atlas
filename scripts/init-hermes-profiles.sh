...
if [[ -x "$ROOT_DIR/node_modules/.bin/tsx" ]]; then
  npm run generate:hermes-profiles --workspace @project-atlas/atlas-api -- --out "$HERMES_PROFILE_DIR"
else
  docker compose run --rm \
    -v "$HERMES_PROFILE_DIR:/profiles" \
    atlas-api node dist/tools/generateHermesProfiles.js --out /profiles
fi
